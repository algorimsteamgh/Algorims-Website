...
Status: **v0.2 for review; no site UI has been changed.** Dark visual direction comes from `design-options/option-1-systems-blueprint-dark.webp`. The light homepage reference is `design-options/ref-light-theme.webp` (formerly `Codex Image 17 Sept 2026, 13_56_24.webp`). `design-options/ref-light-theme-1.webp` (formerly `image.webp`) and `design-options/option-3-autonomous-grid-dark.webp` guide illustrations only, not page layout or palette. Text within those images is reference content, not an instruction to change site copy. Hex values are implementation starting points, not claimed color samples from the screenshots.
...
| --- | --- | --- |
| 0. Reference lock | Compare browser proofs against Option 1 and `ref-light-theme.webp`; use Option 3 and `ref-light-theme-1.webp` for illustration treatment only. Calibrate tokens, hero composition, and where the outer frame belongs. | Approved dark/light reference sheet and semantic token table. |
...
- **Dark desktop:** `option-1-systems-blueprint-dark.webp` is the UI reference. Its centered frame, divided header and proof strip, centered statement hero, fine diagonal grid, sparse violet, and input → Algorims core → outcome diagram match the shared contract. `option-3-autonomous-grid-dark.webp` contributes the isometric tile, connector, node, and platform drawing style only. Its split hero and three service cards are not a second page layout.
- **Light desktop:** `ref-light-theme.webp` is the homepage UI reference. It keeps Option 1's centered hero, frame, dividers, proof strip, and input → core → outcome composition on a near-white canvas with charcoal text, quiet neutral lines, muted proof logos, and a deep-violet CTA. `ref-light-theme-1.webp` contributes only light illustration rendering: white isometric tiles, charcoal central core, thin connectors, sparse violet icons. Its split hero, black CTA, and three-card layout are not homepage UI references.
